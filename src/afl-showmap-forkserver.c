...
u8  *out_dir,                   /* dir with output files                 */
    *in_dir,                         /* dir with input files                      */
    *doc_path,                         /* Path to docs                      */
    *at_file;                          /* Substitution string for @@        */

u32 exec_tmout;                 /* Exec timeout (ms)                 */

u32 total, highest;             /* tuple content information         */

u64 mem_limit = MEM_LIMIT;      /* Memory limit (MB)                 */

s32 dev_null_fd = -1;                  /* FD to /dev/null                   */

u8* in_data;                    /* Input data            */
u32 in_len;                     /* Input data length                 */
...
u8 quiet_mode,                         /* Hide non-essential messages?      */
    ...
    use_stdin = 0,                     /* use stdin - unused here           */
...
static u32 write_results(const char *f) {
  ...
  // if (!strncmp(f, "/dev/", 5)) {//todo

  //   fd = open(f, O_WRONLY, 0600);
  //   if (fd < 0) PFATAL("Unable to open '%s'", f);
  ...
  // } else {
    ...
    unlink(f);                                      /* Ignore errors */
    fd = open(f, O_WRONLY | O_CREAT | O_EXCL, 0600);
    if (fd < 0) PFATAL("Unable to create '%s'", f);
  ...
  if (binary_mode) {
    ...
    ck_write(fd, trace_bits, MAP_SIZE, f);
    close(fd);
...
static void read_input_file(const char *f) {
  ...
  ck_read(fd, in_data, in_len, f);

  close(fd);

  OKF("Read %u byte%s from '%s'.", in_len, in_len == 1 ? "" : "s", f);

}



static void write_to_testcase(const char *f, void* mem, u32 len) {

  s32 fd = out_fd;
  unlink(f);                                     /* Ignore errors. */
  fd = open(f, O_WRONLY | O_CREAT | O_EXCL, 0600);
  if (fd < 0) PFATAL("Unable to create '%s'", f);
  ck_write(fd, mem, len, f);
  close(fd);
}
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+o:+i:f:m:t:A:eqZQUWbcrh")) > 0)
  ...
  char **arg_it = use_argv;
  while (arg_it) {
  	OKF(">> %s\n", *arg_it);
  	arg_it++;
  }
  OKF("<< at_file %s\n", at_file);
  // port from https://github.com/nccgroup/TriforceAFL
  init_forkserver(use_argv);

  d = opendir(in_dir);
  if (!d) FATAL("failed to open input dir");
  mkdir(out_dir, 0755);
  while ((dir = readdir(d)) != NULL) {
    ...
    const char *in_file = alloc_printf("%s/%s", in_dir, basename(dir->d_name));    
    const char *out_file = alloc_printf("%s/%s", out_dir, basename(dir->d_name));
    OKF("running %s\n", in_file);

    read_input_file(in_file);
    write_to_testcase(at_file, in_data, in_len);
    
    run_target(use_argv);
    tcnt = write_results(out_file);
